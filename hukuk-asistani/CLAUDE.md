...
**Gerçek bağımlılık — dürüst ol.** Bu betik pip/pandoc/python-docx gibi bir *ek*
kurulum istemez, ama **Python'un kendisi (python3 veya python) makinede bulunmak
zorundadır** — bu sıfır bağımlılık değildir. macOS ve çoğu Linux dağıtımında
python3 öntanımlı gelir; **Windows'ta genellikle gelmez.** Türkiye'deki büroların
çoğu Windows kullandığı için bunu bir gerçek boşluk olarak kabul et, yokmuş gibi
davranma. Kurulumda (`ilk-kurulum`) bu bir kez test edilip kullanıcıya söylenir;
her skill çalışmasında da aşağıdaki gibi zarifçe kontrol edilir.

**Nasıl:** `.md` dosyasını yazdıktan hemen sonra Bash ile çalıştır — önce
`python3`, yoksa `python` dene:

```
command -v python3 >/dev/null 2>&1 && PY=python3 || (command -v python >/dev/null 2>&1 && PY=python)
$PY "${CLAUDE_PLUGIN_ROOT}/scripts/md_to_docx.py" <yol>.md <yol>.docx
```
...
**Sessizce başarısız olma.** Ne `python3` ne `python` bulunursa veya komut
sıfırdan farklı bir kodla dönerse: kullanıcıya açıkça söyle — "Word kopyası
oluşturulamadı, bu bilgisayarda Python bulunamadı; yalnızca .md kaydedildi. Word
kopyası istersen [python.org](https://python.org)'dan Python kurup (kurulumda
'Add python.exe to PATH' kutusunu işaretleyerek) tekrar dene, ya da .md dosyasını
açıp içeriğini Word'e yapıştır." `.md`'yi yine de teslim et — Word kopyasının
olmaması ana teslimatı engellemez. Asla `.docx` üretilmiş gibi davranma.
