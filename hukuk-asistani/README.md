...
`.docx` üretimi eklentiye gömülü, bağımsız bir betikle yapılır — pandoc gibi ayrı bir program kurmana gerek yok. Tek gerçek şart, bilgisayarında **Python**'un bulunması: Mac ve çoğu Linux'ta zaten kurulu gelir; **Windows'ta genellikle ayrıca kurman gerekir** (ücretsiz, [python.org](https://python.org)'dan iki dakikada). Kurulumda (`ilk-kurulum`) bunu senin için test edip dürüstçe söylerim. Python yoksa hiçbir şey bozulmaz — sözleşme/dilekçe/özet çıktıların yine `.md` olarak gelir, sadece Word ikizi eksik kalır; sonradan Python kurarsan bir sonraki komuttan itibaren otomatik başlar.
...
- **Word (.docx) kopyası gelmiyor, yalnızca .md var** → Bilgisayarında Python yok. Windows'ta bu yaygındır. [python.org](https://python.org)'dan indirip kurulumda "Add python.exe to PATH" kutusunu işaretlemen yeterli — sonrasında yeni komutlardan itibaren Word kopyası otomatik gelir. Eskiden yazılmış `.md` dosyalarını Word'e çevirtmek istersen ilgili skill'i tekrar çalıştırman ya da içeriği kopyala-yapıştır yapman yeterli.
