...
- `/sirketler-hukuku:dosya-calisma-alani new <slug>` — yeni bir dosya oluştur, kısa bir ön görüşme (intake) yap, `matter.md` yaz.
- `/sirketler-hukuku:dosya-calisma-alani list` — dosyaları durumları ve aktif bayraklarıyla listele.
- `/sirketler-hukuku:dosya-calisma-alani switch <slug>` — aktif dosyayı belirle.
- `/sirketler-hukuku:dosya-calisma-alani close <slug>` — dosyayı arşivle (`_archived/` klasörüne taşı, asla silme).
- `/sirketler-hukuku:dosya-calisma-alani none` — aktif dosyadan ayrıl, sadece genel pratik seviyesinde çalış.

## Talimatlar

1. `~/.claude/plugins/config/claude-for-legal-turkish/sirketler-hukuku/CLAUDE.md` dosyasını oku — `## Dosya çalışma alanları` (Matter workspaces) bölümünü kontrol et. Eğer `Etkin` (Enabled) değilse (`✗`), kullanıcıya söyle: "Dosya çalışma alanları kapalı — tek müşterili bir şirket-içi (in-house) pratik olarak yapılandırılmışsınız. Eğer birden fazla müşteriyle çalışıyorsanız `/sirketler-hukuku:ilk-kurulum --redo` çalıştırıp özel pratik (büro) ayarını seçin. Aksi takdirde bu yeteneğe ihtiyacınız yok." Hata verme — şirket-içi kullanıcılar için kapalı olması beklenen durumdur.
...
```
~/.claude/plugins/config/claude-for-legal-turkish/sirketler-hukuku/
...
Örnek slug'lar: `abc-msa-2026`, `xyz-birlesme`, `tedarikci-nda`.
...
2. Ön görüşme (intake) yap:
   ...
   - **Dosya türü (Matter type)** (M&A alıcı taraf | M&A satıcı taraf | finansman | yönetim kurulu meselesi | vb.)
   - **Gizlilik seviyesi** (standart | yüksek | clean-team)
   - **Kilit olaylar (Key facts)** (2-5 cümle: Bu dosya ne hakkında?)
   - **Dosyaya özel geçersiz kılmalar (Overrides)** (Örn: "Sorumluluk üst sınırı (LoL) 24 ay olmalı", "Karşı taraf stratejik ortak - ılımlı dil kullanılmalı")
...
## Taraflar
**Müşteri:** [isim]
**Karşı Taraf:** [isim(ler)]

## Dosya türü
[M&A alıcı | M&A satıcı | finansman | ...]

## Kilit olaylar
[2-5 cümle. Dosyanın konusu ve önemi.]

## Dosyaya özel geçersiz kılmalar (Overrides)
[Örn: "Tahkim şartı İTO olmalı", "Uygulanacak Hukuk: İngiliz Hukuku"]
```
